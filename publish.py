...
Then it restarts the receiver and checks /api/health. The token (`/etc/osmsfm/token`) is created
only when it does not exist yet, and printed at the end so it can be handed out to friends.
...
def main() -> int:
    ...
    ssh(
        ...
        # token: only when it does not exist yet; www-data must be able to read it, nobody else
    ...
    print("  token for the trackers:", lines[2] if len(lines) > 2 else "?")
    return 0
