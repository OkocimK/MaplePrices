...
Skleja `shopframe` (geometria), winocr (nazwy, tytuł sklepu, minimapa), `glyphs` (ceny)
i `names` (kanoniczna nazwa scrolla). Nic tu nie dotyka gry ani dysku.
...
def recognize(frame: Image.Image, cursor: tuple[int, int] | None = None) -> Result:
    ...
    if not fr.open:
        return Result(False)
    title = ocr(fr.title)
    res = Result(True, owner=parse_title(title), title=title)
    res.timer_raw = ocr(frame.crop(sf.TIMER))
    res.ttl_min = parse_timer(res.timer_raw)
    res.map_raw = ocr(frame.crop(MINIMAP), scale=2)
    ...
    for r in fr.rows:
        ...
        name_box = sf._box(y0, sf.NAME)
        price_box = sf._box(y0, sf.PRICE)
        ...
        crop = frame.crop((sf.ICON[0], y0, sf.NAME[2], y0 + sf.ROW_PITCH))
        if sc is not None:
    ...
    for pat in sys.argv[1:]:
        for f in sorted(glob.glob(pat)):
            ...
            if not r.open:
                ...
                continue
            print(f"{f}: {r.owner!r} @ {r.map}<{r.channel}> znika za {r.ttl_min} min ({r.timer_raw!r})  pominięte: kursor {r.skipped_cursor}, cena {r.skipped_price}, nieświeże ikony {r.skipped_stale}")
            for o in r.obs:
